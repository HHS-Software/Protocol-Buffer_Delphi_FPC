...
###  Notes:
This utility can process hundreds of input .proto files in the same run.  It searches directories for .proto files and will output one .pas file for every .proto file.
- For better results, use the ```-m, -merge``` param, as it merges hundreds of input files into one large .pas file.  All the required ```uses, forwards, etc``` statements are added automatically.
- This utility uses a basic class template to create .pas files.  The default template file is attached to this utility, and is used automaticaly.  To modify the template, dump the original attached one with ```-prntmplt```, modify it and use it with ```-tmplt=NAME``` parameter.
- The ```-pr, -prefix=ID``` adds a short name to the front of all proto classes. i.e. ```prefix=ABC``` gives ```TABC<protoname>Proto = class```
...
- The option ```-sv, -srcver=NAME``` gives a single line import of a version number.  It's usefull to stamp proto file revisions # into the resulting .pas files.  It's printed on about line 6 in the top comments.
- The ```-valeq``` parameter controls how ```property write``` code is created.  In default, every property value assignment (```Someprop := 0;```) will write the value to the private FSomeprop: Integer;  This is not normal pascal.  With the ```-valeq``` param added, the paramater write procedure code will include the usual ```if Value <> FSomeprop then FSomeprop := Value;```\
This setting is important to the way it sends data.  Writing a property value will set the touched flag, and cause the value to be pushed to the data stream, including empty strings and zeros.  However, depending on the recieving end code, they might follow the proto specification rules and know that zeroes and null strings are valid field values, and do not need them sent (use ```-valeq```).  But with badly designed receiver code, they will expect all fields to be transmitted (ignoring the optional status of the proto field) and check fields with a ```set / has_value``` test including the zeroes and nulls (do not use ```-valeq```).
